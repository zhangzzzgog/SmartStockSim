...
class LSTM_LongModel:
    ...
    def train(self):
        ...
        n_test       = args.n_test       # 测试集数目

        # 新闻情绪评估路径
        attitude_dir = "attitude/" + stock_name + "attitude.csv"
        ...
        attitude_data = pd.read_csv(attitude_dir)  # 读取情绪数据
        data = pd.merge(data, attitude_data, on="date", how="left")  # 将“attitude”数据合并到股票数据中，按“date”对齐
        data.insert(2, "attitude", data.pop("attitude"))  # 将 attitude 列插入到第3列位置（索引为2）
        data = data.fillna(0)  # 用0填充空值
        data_rows, data_columns = data.shape

        shape_num = 2
        index_max = 1 + shape_num

        # 数据集划分
        training_set = data.iloc[0:data_rows - n_test, 1:index_max].values  
        test_set     = data.iloc[data_rows - n_test:, 1:index_max].values
        ...
        X_train          = X_train.reshape(X_train.shape[0], X_train.shape[1], shape_num) # 最后一个数字是特征数，包含开盘、收盘和情绪3个特征

        X_test, y_test   = data_split(testing_set_scaled, n_timestamp)
        X_test           = X_test.reshape(X_test.shape[0], X_test.shape[1], shape_num)
        ...
        # 计算样本权重，越新的数据权重越高
        if ttype == 1:
            time_weights = np.arange(1, train_length + 1)
            time_weights = time_weights / time_weights.sum()  # 归一化，使权重和为1
        if ttype == 2:
            tlist = []
            for i in range(1,train_length + 1):
                tlist.append(1 / (train_length + 2 - i))
            time_weights = np.asarray(tlist, dtype=float)
        ...
        if model_type == 1:
            ...
            model.add(LSTM(units=50, activation='relu',
                        input_shape=(X_train.shape[1], shape_num)))
        ...
        # 预测数据只有一个特征，即股票价格
        # 创建一个与训练数据相同形状的数组，第二个特征用零填充，否则反归一化会失败
        predicted_stock_price = np.hstack((predicted_stock_price, np.zeros((predicted_stock_price.shape[0], 1))))
        ...
        # 画出真实数据和预测数据的对比曲线
        plt.plot(real_stock_price[:,0], color='red', label='Stock Price')
        plt.plot(predicted_stock_price[:,0], color='blue', label='Predicted Stock Price')
        ...
        predicted_future = []
        last_attitude = data.iloc[-1][2]

        # 循环预测未来 30 天的股票价格
        for _ in range(future_days):
            # 使用模型预测下一个值
            next_price = model.predict(last_sequence.reshape(1, n_timestamp, shape_num))  # reshape to 3D

            # 规格化预测值
            # 扩展 next_price 的形状，使其匹配 last_sequence 的形状
            # 将 next_price 从 (1, 1) 扩展为 (1, 2) 
            # 扩展情绪值以最后一天的值为准
            next_price = np.array([[next_price[0][0], last_attitude]])
            # 将预测的值保存
        ...
        predicted_future = np.hstack((predicted_future, np.full((predicted_future.shape[0], 1), last_attitude)))
        ...
        for days in range(1, predicted_future.size):
            ...
            pricelist.append(predicted_future[days])
